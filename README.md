...
导入印章时录入真实直径（公章一般 40/42mm，拿尺子量一下你的章），之后盖出来的每一个章打印出来都和真章分毫不差。扫描件歪歪扭扭、手机拍照带黑边？没关系，纸边校准伺候：程序先用 OpenCV 自动框出纸面，你拖四个角微调一下就行，不用自己从头点四下。
...
- **校准**：自动框出纸面 → 拖四个角微调 → 透视拉正为 A4；拖动时实时显示长宽比，偏离 A 系纸会提示；全程可平移缩放，方向键微调 0.1mm
...
- 自动纸边检测靠纸/背景亮度差。纸铺满画面的无边扫描件会直接框住整幅图（这是对的）；实在框不出来时会给一个默认框让你拖，不会把你丢回从零点四下
